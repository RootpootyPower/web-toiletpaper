...
function init() {
    ...
    for (let i = 1; i < length; i++) {
        let img = gallery.children.item(i);
    ...
    input1 = controls1.children[1];
    input2 = controls2.children[1];
...
function changeImage(image) {
    ...
    let currentImg = gallery.children.item(current-1);
    if (currentImg == null) {
        console.error("current image not found!");
        return;
    }
    let newImg = gallery.children.item(image-1);
    if (newImg == null) {
